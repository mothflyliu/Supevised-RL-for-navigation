    ...
    def __init__(self):
      ...
      while not rospy.is_shutdown():
        ...
        if(self.stateMsg.state == self.stateMsg.S2):
          ...
          if(self.pub_twist((3.5,0.5),-90)):
            self.actionMsg.ACTION = self.actionMsg.THIRD
            self.action_pub.publish(self.actionMsg)
            self.start_timing()
            
        if(self.stateMsg.state == self.stateMsg.S3):
          if(self.pub_twist((6.5,0.5),0)):
            self.actionMsg.ACTION = self.actionMsg.FORTH
            self.action_pub.publish(self.actionMsg)
            self.start_timing()
  
        if(self.stateMsg.state == self.stateMsg.S4):
          if(self.pub_twist((6.5,4.5),90)):
            self.actionMsg.ACTION = self.actionMsg.FIFTH
            self.action_pub.publish(self.actionMsg)
            self.start_timing()       

        if(self.stateMsg.state == self.stateMsg.S5):
          if(self.pub_twist((3.5,4.5),180)):
            self.actionMsg.ACTION = self.actionMsg.SIXTH
            self.action_pub.publish(self.actionMsg)
            self.start_timing()

        if(self.stateMsg.state == self.stateMsg.S6):
          if(self.pub_twist((3.5,0.5),-90)):
            self.actionMsg.ACTION = self.actionMsg.SEVENTH
            self.action_pub.publish(self.actionMsg)
            self.start_timing()
  
        if(self.stateMsg.state == self.stateMsg.S7):
          if(self.pub_twist((0.5,0.5),180)):
            self.actionMsg.ACTION = self.actionMsg.EIGHTH
            self.action_pub.publish(self.actionMsg)
            self.start_timing()
