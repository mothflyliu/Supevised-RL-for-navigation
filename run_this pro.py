...
from environment_3_prob  import Env
...
class QLearningTable(object):
    def __init__(self, state_tran, env_transitions,actions=[0,1,2,3],learning_rate=0.01, reward_decay=0.9, e_greedy=0.2): #e_greedy=0.9
    ...
    def getAction(self, state):
        ...
        if np.random.rand()<self.epsilon:
            ...
            state_action = state_action.loc[self.actions_list]
    ...
    def learn(self, s, a, r, s_):
        ...
        if s_ != [5.5, 4.5]:
            ...
            q_target = r + self.gamma * (self.q_table.loc[s_, :].loc[self.actions_list].max())  # next state is not terminal
        else:
            q_target = r
...
if __name__ == '__main__':
    ...
    for e in range( EPISODES):
        ...
        if step_num % 5 ==0:
                agent.save_model()
        if e>25 and agent.epsilon < 0.99: 
                agent.epsilon += 0.1     
        print(e,step_num,score)
    savedata.SaveToTxt()
    print(result1)
    show_plot(result1)
